-- 🎯 改进猜拳游戏超时检查逻辑
-- 问题：之前的超时检查只检查 created_at，对于 playing 状态的房间，
--       应该检查是否有残留的长时间未完成的游戏（超过5分钟）
-- 修复：改进 check_rps_timeout 函数，增加对长时间残留 playing 状态房间的检查
...
DECLARE
  ...
  v_timeout_long_playing TIMESTAMP := NOW() - INTERVAL '5 minutes'; -- 🎯 新增：长时间残留检查（5分钟）
  v_room RECORD;
...
BEGIN
  ...
  -- 🎯 3. 新增：处理长时间残留的 playing 状态房间（超过5分钟）
  -- 这种情况可能是由于系统错误或玩家没有完成游戏导致的
  FOR v_room IN
    SELECT * FROM rps_rooms
    WHERE status = 'playing'
      AND created_at < v_timeout_long_playing
      AND created_at >= v_timeout_playing -- 避免重复处理上面的情况
    FOR UPDATE SKIP LOCKED
  LOOP
    -- 退回双方本金
    UPDATE profiles
...
-- 注释
COMMENT ON FUNCTION public.check_rps_timeout() IS '检查并处理超时的猜拳房间：等待加入30秒超时，出手阶段60秒超时，长时间残留5分钟超时';
