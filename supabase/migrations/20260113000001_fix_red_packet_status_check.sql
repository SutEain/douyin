-- 🎯 修复红包状态检查逻辑，添加更详细的错误信息
-- 问题：用户看到"剩余 39/88 份"，但点击时提示"已领完"
-- 原因：RPC 函数检查 status <> 'active' 时返回的错误信息不够详细
...
BEGIN
  ...
  IF v_packet.status <> 'active' THEN
    ...
    BEGIN
      IF v_packet.status = 'completed' THEN
        v_error_msg := '🎈 来晚了，红包已被抢光（状态：已完成）';
      ELSIF v_packet.status = 'expired' THEN
        v_error_msg := '⏰ 红包已过期（状态：已过期）';
      ELSE
        v_error_msg := '❌ 红包已失效或已结束（状态：' || v_packet.status || '）';
      END IF;
      
      -- 🎯 记录详细日志到数据库（用于调试）
      RAISE NOTICE '[RedPacket-RPC] ❌ 状态检查失败: packet_id=%, status=%, remaining_count=%, total_count=%', 
        p_packet_id, v_packet.status, v_packet.remaining_count, v_packet.total_count;
      
      RETURN QUERY SELECT false, v_error_msg, 0::DECIMAL(10,2), false;
      RETURN;
    END;
  END IF;
  ...
  IF v_packet.remaining_count <= 0 THEN
    -- 🎯 记录详细日志
    RAISE NOTICE '[RedPacket-RPC] ❌ 剩余数量检查失败: packet_id=%, remaining_count=%, status=%', 
      p_packet_id, v_packet.remaining_count, v_packet.status;
...
-- 更新注释
COMMENT ON FUNCTION claim_group_red_packet IS '领取群红包，支持手气最佳统计。已修复：添加详细错误信息和日志，便于调试状态不一致问题';
