-- 🎯 修复：保护封号字段（is_banned, ban_reason）不被非法修改
-- 问题：触发器函数没有保护 is_banned 和 ban_reason 字段
-- 修复：在触发器函数中添加对这些字段的保护，只允许管理员通过专门的管理函数修改
...
DECLARE
    ...
    v_is_admin_ban_operation BOOLEAN := false;
BEGIN
    -- 🎯 service_role: 完全允许（但统计字段和封号字段仍需要通过专门的操作）
    IF current_user = 'service_role' THEN
        ...
        -- 🛑 检查是否是管理员封号操作
        BEGIN
            v_is_admin_ban_operation := current_setting('app.admin_ban_operation', true)::boolean;
        EXCEPTION
            WHEN OTHERS THEN
                v_is_admin_ban_operation := false;
        END;
        ...
        -- 🛑 如果不是管理员封号操作，锁定封号字段
        IF NOT v_is_admin_ban_operation THEN
    ...
    -- 🎯 检查是否是管理员（允许管理员修改auto_approve等字段，但不允许修改统计字段和封号字段）
    BEGIN
    ...
    IF current_user = 'postgres' THEN
        ...
        -- 🎯 如果是资产继承操作，直接允许修改余额，但不允许修改 is_admin、统计字段和封号字段
        IF v_is_inheritance THEN
            -- 🛑 即使资产继承，也不允许修改 is_admin、统计字段和封号字段
        ...
        -- 如果是 PC28 结算操作，允许修改余额，但不允许修改 is_admin、统计字段和封号字段
        IF v_is_pc28_settlement THEN
            -- 🛑 即使PC28结算，也不允许修改 is_admin、统计字段和封号字段
            IF NEW.is_admin IS DISTINCT FROM OLD.is_admin THEN
                NEW.is_admin := OLD.is_admin;
            END IF;
            NEW.follower_count := OLD.follower_count;
        ...
        -- 🎯 非 PC28 结算且非资产继承：只允许修改自己的余额，但不允许修改 is_admin、统计字段和封号字段
        -- 检查是否在修改资产字段
    ...
    -- 🎯 其他授权用户（管理员等）- 但这里也不应该允许修改 is_admin、统计字段和封号字段
    -- 注意：即使是管理员，也不应该通过普通UPDATE修改 is_admin、统计字段和封号字段
    IF current_user IN ('supabase_admin', 'dashboard_user') OR 
       EXISTS (
        ...
        -- 🛑 即使是授权用户，也不允许修改 is_admin、统计字段和封号字段
        IF NEW.is_admin IS DISTINCT FROM OLD.is_admin THEN
            NEW.is_admin := OLD.is_admin;
        END IF;
        -- 🛑 管理员也不允许直接修改统计字段和封号字段（必须通过触发器或专门函数）
        NEW.follower_count := OLD.follower_count;
        NEW.following_count := OLD.following_count;
        NEW.total_likes := OLD.total_likes;
        NEW.video_count := OLD.video_count;
    ...
    -- 🛑 默认情况：锁定所有敏感字段（包括 is_admin、统计字段、封号字段）
    -- 🎯 但如果用户是管理员，允许修改auto_approve
    IF v_is_admin THEN
        -- 管理员可以修改auto_approve，但不能修改is_admin、统计字段、封号字段和其他敏感字段
        IF NEW.is_admin IS DISTINCT FROM OLD.is_admin THEN
            NEW.is_admin := OLD.is_admin;
        END IF;
        -- 🛑 管理员也不允许直接修改统计字段和封号字段
        NEW.follower_count := OLD.follower_count;
        NEW.following_count := OLD.following_count;
        NEW.total_likes := OLD.total_likes;
        NEW.video_count := OLD.video_count;
    ...
    -- 🛑 非管理员：锁定所有敏感字段（包括 is_admin、auto_approve、统计字段、封号字段）
    NEW.is_admin := OLD.is_admin;
    NEW.auto_approve := OLD.auto_approve;
    NEW.balance_coins := OLD.balance_coins;
    NEW.frozen_coins := OLD.frozen_coins;
...
COMMENT ON FUNCTION public.protect_sensitive_profile_fields IS '🚨 修复：保护封号字段（is_banned, ban_reason）不被非法修改，只允许通过管理员封号操作修改';
