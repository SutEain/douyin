-- 🚨 紧急修复：加强record_video_view_v2函数的安全验证
-- 问题：脚本可能用多个账号同时刷，或者删除watch_history后重新刷
-- 修复：
-- 1. 添加更严格的频率限制（同一用户对同一视频，1分钟内只能调用1次）
-- 2. 添加IP级别的验证（需要在Edge Function层实现）
-- 3. 添加异常检测（播放量增长异常时记录警告）
...
COMMENT ON FUNCTION public.record_video_view_v2 IS '🚨 修复安全漏洞：移除anon权限、加强频率限制（1分钟1次）、添加异常检测，防止脚本刷播放量';
