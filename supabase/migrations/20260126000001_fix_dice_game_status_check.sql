-- 🎯 修复骰子游戏状态检查问题
-- 问题：settle_dice_room 期望状态为 waiting，但 join_dice_room 在满员时将状态改为 rolling
-- 解决方案：允许 settle_dice_room 接受 rolling 状态
...
BEGIN
    ...
    IF v_room.status NOT IN ('waiting', 'rolling') THEN
        RETURN json_build_object('success', false, 'message', format('房间状态不正确: %s，期望: waiting 或 rolling', v_room.status));
    END IF;
...
COMMENT ON FUNCTION public.settle_dice_room IS '🎯 修复：允许 waiting 或 rolling 状态，解决状态不匹配问题';
