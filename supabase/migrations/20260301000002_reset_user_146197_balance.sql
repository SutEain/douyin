...
BEGIN
    ...
    WHERE numeric_id = 146197;

    IF v_user_id IS NULL THEN
        RAISE EXCEPTION '用户 numeric_id 146197 不存在';
    END IF;

    IF v_old_balance = 0 THEN
        RAISE NOTICE '用户 146197 余额已是 0，无需操作';
        RETURN;
    END IF;
    ...
    INSERT INTO public.coin_transactions (
        ...
        v_new_balance,
        'adjustment',
        format('[系统操作] 余额清零：原余额 %s 抖币（用户 146197）', v_old_balance),
        NULL
    );

    RAISE NOTICE '用户 146197 余额已清零：% -> 0', v_old_balance;
END $$;
