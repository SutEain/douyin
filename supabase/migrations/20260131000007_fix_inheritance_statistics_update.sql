-- 🎯 修复：资产继承函数更新统计字段时需要设置会话变量
-- 问题：admin_execute_inheritance 函数更新 video_count 时被保护触发器阻止
-- 修复：在更新统计字段前设置 app.allow_statistics_update 会话变量
...
BEGIN
    ...
    IF v_source_balance > 0 THEN
        ...
        -- 记录转出交易（已移除 admin_id 字段，管理员信息放入 description）
        ...
        -- 记录转入交易（已移除 admin_id 字段，管理员信息放入 description）
        INSERT INTO public.coin_transactions (
            user_id, type, amount, balance_after, description
        ) VALUES (
            v_target_profile_id,
...
COMMENT ON FUNCTION public.admin_execute_inheritance IS '🚨 修复：添加会话变量标识，允许触发器修改余额和统计字段';
