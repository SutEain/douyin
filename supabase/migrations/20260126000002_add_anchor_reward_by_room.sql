...
DECLARE
    ...
    v_room_reward RECORD; -- 房间奖励记录（用于按房间分组统计）
BEGIN
    -- 🎯 设置会话变量，允许修改用户余额
    ...
    FOR v_room_reward IN
        SELECT 
            r.anchor_id,
            r.id as room_id,
            COALESCE(SUM(b.amount), 0) as room_total_bet
        FROM public.live_broadcast_rooms r
        LEFT JOIN public.pc28_bets b ON b.room_id = r.id 
            AND b.global_round_id = p_global_round_id 
            AND b.status = 'settled'
        WHERE EXISTS (
            SELECT 1 FROM public.pc28_bets 
            WHERE global_round_id = p_global_round_id 
            AND room_id = r.id 
            AND status = 'settled'
        )
        GROUP BY r.id, r.anchor_id
...
COMMENT ON FUNCTION public.settle_global_round IS '结算全局期数：按房间分组，每个房间主播获得该房间下注额的1%。规则：13/14时组合玩法仅回本；对子/顺子/豹子时组合玩法正常结算。在谁的直播间下注，才给谁奖励。';
