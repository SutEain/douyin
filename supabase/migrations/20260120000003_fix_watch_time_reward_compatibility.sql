-- 🚨 修复观看时长奖励兼容性问题
-- 问题：改之前的交易记录格式是 "观看时长奖励: 30min"，改之后是 "观看时长奖励: 观看30分钟 (watch_time_30min, ...)"
-- 导致新函数无法识别旧格式的已领取记录，用户可以重复领取
-- 修复：同时检查旧格式（: 5min/: 15min/: 30min）和新格式（watch_time_5min/watch_time_15min/watch_time_30min）

-- 1. 修改获取观看时长奖励状态函数（兼容旧格式）
...
COMMENT ON FUNCTION public.claim_watch_time_reward IS '🚨 修复兼容性问题：同时检查新格式（watch_time_xxx）和旧格式（: 5min/: 15min/: 30min），防止重复领取';
COMMENT ON FUNCTION public.get_watch_time_reward_status IS '🚨 修复兼容性问题：同时检查新格式（watch_time_xxx）和旧格式（: 5min/: 15min/: 30min），防止重复领取';
