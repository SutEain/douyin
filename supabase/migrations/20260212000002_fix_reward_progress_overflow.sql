-- 🎯 修复奖励进度值超过当前总播放量/获赞数的问题
-- 问题：用户80000的进度值(18351)超过了当前总播放量(18241)，导致待领取显示为0
-- 解决：当进度值超过当前总播放量时，自动重置进度值为当前总播放量
...
COMMENT ON FUNCTION public.get_author_reward_stats(UUID) IS 
    '获取作者奖励统计数据（供 Bot 界面展示）。只返回 is_active = TRUE 的任务规则。如果任务被关闭，对应的 stats 将返回 NULL。自动修复进度值超过当前总数的情况。';
