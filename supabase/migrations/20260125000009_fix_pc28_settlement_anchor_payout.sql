-- 🚨 修复PC28结算逻辑：主播赔付金额计算错误
-- 问题：v_anchor_payout = v_bet.amount - v_user_gain 导致负数
-- 正确逻辑：主播需要支付奖金总额（v_payout），因为用户下注金额已在用户下注时扣除
...
BEGIN
    ...
    IF v_anchor_balance < v_total_payout THEN
        ...
        RETURN json_build_object(
            'success', false,
            'message', format('主播余额不足，无法结算。需要：%s，当前：%s', v_total_payout, v_anchor_balance),
    ...
    LOOP
        ...
        IF v_is_win THEN
            ...
            -- 🚨 修复：主播需要支付奖金总额（v_payout），因为用户下注金额已在用户下注时扣除
...
COMMENT ON FUNCTION public.settle_pc28_round IS '🚨 修复：主播需要支付奖金总额（v_payout），而不是 v_bet.amount - v_user_gain';
