...
import { shortDramaVideoTab } from '@/api/videos'
...
const vIsCanPlay = {
  mounted(el, binding) {
    ...
    console.log('[ShortDramaTab] v-is-can-play mounted', {
  ...
  unmounted(el) {
    console.log('[ShortDramaTab] v-is-can-play unmounted', {
      videoSrc: el.currentSrc?.substring(0, 50) || 'no src',
...
<template>
  <div class="video-tab" @dragstart="(e) => _stopPropagation(e)">
    <ScrollList class="Scroll" v-if="state.show" :api="shortDramaVideoTab" :show-scroll-loading="false">
      <template v-slot="{ list, loading }">
        ...
        <div v-else-if="!loading" class="empty">暂无短剧</div>
